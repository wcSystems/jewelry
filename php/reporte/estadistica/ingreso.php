<?php
	...
	$DESDE = $util->tosqldate($_POST["DESDE"]);
	$HASTA = $util->tosqldate($_POST["HASTA"]);
	$cont = 0;
	$a = date_create($DESDE);
	$b = date_create($HASTA);
	$diff = date_diff($a,$b);
	...
	while( $cont < $diff ){
		...
		$ingreso = (array) $query->select(array("SUM(MONTO) as MONTO"),"MOVIMIENTO","where TIPO = 1 and MONEDA = 1 and FECHA <= '".$DESDE."'")[0];
		...
		$egreso = (array) $query->select(array("SUM(MONTO) as MONTO"),"MOVIMIENTO","where TIPO = 2 and MONEDA = 1 and FECHA <= '".$DESDE."'")[0];
		...
		$ingreso = (array) $query->select(array("SUM(MONTO) as MONTO"),"MOVIMIENTO","where TIPO = 1 and MONEDA = 2 and FECHA <= '".$DESDE."'")[0];
		...
		$array[] = array("FECHA"=>$DESDE,"BOLIVARES"=>$Bingreso,"DOLARES"=>$Dingreso);
