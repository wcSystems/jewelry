				...
				<div class="col-md-6 smart-form no-padding">
					<section class="col col-6 no-margin ">
						<label class="input">
							...
							<select id="rango" style="text-align-last: center;" class="input-sm col-12 nocaret">
					...
					<section class="col col-6 no-margin">
						<label class="input">
							...
							<input type="text" class="" id="valor" value="0">
							<i class="tooltip tooltip-top-right">Dias</i>
						</label>
					</section>
				</div>
...
pageSetUp();
	...
	function buscar(){
		...
		$.ajax({
			url:"php/reporte/contrato/lista.php",
			type:"POST",
			data:{"VALOR":valor,"RANGO":rango},
			success: function(resp){
				...
				$.each(resp["MENSAJE"],function(indice,valor){
					...
					$("#dt_basic").dataTable().fnAddData([
						...
						valor["DIAS"],
						mensaje,
						"<button class='btn btn-primary' href='ajax/reporte/modal/contrato.php?ID="+valor["CONTRATO"]+"' data-toggle='modal' data-target='#detalle'><i class='fa fa-search'></i></button>"
					]);
				});
			}
		});
