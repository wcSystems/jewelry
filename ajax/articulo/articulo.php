...
<div class="row">
	<div class="col-md-12">
		<div class="well well-sm">
			<button href="ajax/articulo/modal/articulo.php" data-toggle="modal" data-target="#detalle" data-placement="right" rel="tooltip" data-original-title="Nuevo articulo" class="btn btn-success"><i class="fa fa-plus"></i></button>
		</div>		
	</div>
</div>
<div class="row">
...
pageSetUp();
	...
	var medida = JSON.parse('<?php echo json_encode($medida); ?>');
	...
	function buscar(){
		$.ajax({
			url:"php/articulo/articulo/lista.php",
			success: function(resp){
				...
				$.each(resp["MENSAJE"],function(indice,valor){
					$("#dt_basic").dataTable().fnAddData([
						...
						valor["DESCRIPCION"],
						medida[valor["MEDIDA"]]["DESCRIPCION"],
						"<button class='btn btn-primary' href='ajax/articulo/modal/articulo.php?ID="+valor["ID"]+"' data-toggle='modal' data-target='#detalle'><i class='fa fa-search'></i></button>",
	...
	function elim(id){
		$.ajax({
			url: "php/articulo/articulo/elim.php",
			type: "POST",
			data:{"DATO":id},
			success: function(resp){
				//console.log(resp);
				alerta('success','Articulo eliminado');
				buscar();
			}
		});
	}
