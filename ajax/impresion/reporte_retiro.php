...
$temp = (array) $query->select(array("*"),"VRETIRO","WHERE ID = ".$_GET["ID"]);
$count = (array) $query->select(array("count(ID) as COUNT"),"VRETIRO","WHERE ID = ".$_GET["ID"]);
$sum = (array) $query->select(array("SUM(DIMENSION) as SUM"),"VRETIRO","WHERE ID = ".$_GET["ID"]);?>
<!DOCTYPE html>
<html lang="en-us">	
	<head>
		<meta charset="utf-8">
	...
	<body class="content">
		<div class="row" style="border:1px solid black;padding:3%">
			<div class="col-md-6" style="display:inline-block;width:50%;float:left">
				...
				<br><br>
				<label>RETIRO N° <?php echo $_GET["ID"]; ?></label>
			</div>
