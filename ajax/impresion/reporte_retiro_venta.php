...
$temp = (array) $query->select(array("*"),"VRETIROV","WHERE ID = ".$_GET["ID"]);
$id = (array) $query->select(array("ID"),"VRETIROV","ORDER BY ID DESC LIMIT 0,1");
$count = (array) $query->select(array("count(ID) as COUNT"),"VRETIROV","WHERE ID = ".$_GET["ID"]);
$sum = (array) $query->select(array("SUM(DIMENSION) as SUM"),"VRETIROV","WHERE ID = ".$_GET["ID"]);?>
<!DOCTYPE html>
<html lang="en-us">	
	<head>
		<meta charset="utf-8">
