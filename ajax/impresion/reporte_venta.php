...
$temp = (array) $query->select(array("*"),"VVENTA","WHERE ID = ".$_GET["ID"]);
$count = (array) $query->select(array("count(ID) as COUNT"),"VVENTA","WHERE ID = ".$_GET["ID"]);
$sum = (array) $query->select(array("SUM(MONTO) as SUM"),"VVENTA","WHERE ID = ".$_GET["ID"]);?>
<!DOCTYPE html>
<html lang="en-us">	
	<head>
		<meta charset="utf-8">
                                                ...
                                                <table style="margin-bottom:10px;border-radius:5px;border: 1px solid #000 !important" class="table table-condensed table-striped table-bordered table-hover" width="100%">
													...
													<tbody>			                
														<tr>
                                                            ...
                                                            <td style="border-right: 0px !important;border-bottom: 0px !important;width:50%;text-align:center !important" class="center"> <?php echo 'N° ' . $_GET["ID"]; ?></td>
														</tr>
													</tbody>
												</table>
