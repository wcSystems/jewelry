...
$temp = (array) $query->select(array("*"),"VRETIROV","WHERE ID = (SELECT ID FROM VRETIROV ORDER BY ID DESC LIMIT 0,1)");
$id = (array) $query->select(array("ID"),"VRETIROV","ORDER BY ID DESC LIMIT 0,1");
$count = (array) $query->select(array("count(ID) as COUNT"),"VRETIROV","WHERE ID = (SELECT ID FROM VRETIROV ORDER BY ID DESC LIMIT 0,1)");
$sum = (array) $query->select(array("SUM(DIMENSION) as SUM"),"VRETIROV","WHERE ID = (SELECT ID FROM VRETIROV ORDER BY ID DESC LIMIT 0,1)");?>
<!DOCTYPE html>
<html lang="en-us">	
	<head>
		<meta charset="utf-8">
	...
	<body class="content">
		<div class="row" style="border:1px solid black;padding:3%">
			<div class="col-md-6" style="display:inline-block;width:50%;float:left">
				...
				<br><br>
				<label>Detalle de venta N° <?php echo $id[0]['ID']; ?></label>
			</div>
			...
			<br><br>
			<label>TOTAL DE PIEZAS: <?php echo $count[0]['COUNT'] . '<br>'; ?></label>
			<label>TOTAL EN GRAMOS: <?php echo number_format($sum[0]['SUM'],2); ?></label>
		</div>
